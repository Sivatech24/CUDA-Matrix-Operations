...
int main() {
    ...
    fillRandomMatrix<<<64, 256>>>(d_matrix, time(NULL));
    cudaDeviceSynchronize();

    std::cout << "Random 256x256 Matrix with values from 0 to " << MATRIX_SIZE - 1 << " at random positions:\n";
    displayMatrix<<<64, 256>>>(d_matrix);
    cudaDeviceSynchronize();
