...
#define N 512  // Matrix size: 512x512
...
int main() {
    ...
    fillRandomMatrix<<<128, 256>>>(d_matrix, time(NULL));
    cudaDeviceSynchronize();

    std::cout << "Random 512x512 Matrix with values from 0 to " << MATRIX_SIZE - 1 << " at random positions:\n";
    displayMatrix<<<128, 256>>>(d_matrix);
    cudaDeviceSynchronize();
